...
void setup() {
  ...
  server.begin();
  Serial.println("ESP32 C Server started");
  ...
  if (request == "STATUS") {
    ...
    if (commaIndex > 0) {
      ...
      if (validPin && (value == 0 || value == 1)) {
        ...
        return "C:SET_PIN_" + String(pin) + "_TO_" + String(value);
      }
    }
    return "C:ERROR_INVALID_COMMAND";
  }
  
  return "C:ERROR_UNKNOWN_COMMAND";
}