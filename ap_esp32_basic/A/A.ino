...
// Pin configuration for ESP32 A
int controlPins[] = {4, 5, 12, 13, 14, 16, 17, 18, 19, 21};
...
void setup() {
  ...
  WiFi.config(IPAddress(192, 168, 0, 101), IPAddress(192, 168, 0, 1), IPAddress(255, 255, 255, 0));
  ...
  server.begin();
  Serial.println("ESP32 A Server started");
  ...
  if (request == "STATUS") {
    ...
    if (commaIndex > 0) {
      ...
      if (validPin && (value == 0 || value == 1)) {
        ...
        return "A:SET_PIN_" + String(pin) + "_TO_" + String(value);
      }
    }
    return "A:ERROR_INVALID_COMMAND";
  }
  
  return "A:ERROR_UNKNOWN_COMMAND";
}